...
from PyQt5.QtCore import QDate, Qt, QThread, QTimer, QUrl
...
class View:
    ...
    def showAdvancedSearchScreen(self, orderForm, selector):
        self.advancedOrderScreen = AdvancedOrderScreen(
            self.model, self, orderForm, selector
        )
...
class SettingsManageTechnicianForm(QMainWindow):
    ...
    def handleAddTechPressed(self):
        ...
        user = self.user.text()
        if (
            self.pswd.text() == self.confirmPswd.text()
            and self.pswd.text()
            and self.confirmPswd.text()
        ):
            if self.fName.text() and self.lName.text() and self.user.text():
                if self.model.findTechUsername(self.user.text()) == None:
                    ...
                    self.view.auditor(
                        currentTech, "Add", user, "Settings_Edit_Technician"
                    )
                else:
                    self.errorMessage.setStyleSheet(
                    ...
                    self.errorMessage.setText(
                        "A technician with this username already exists"
                    )
            else:
                self.errorMessage.setStyleSheet(
                    "font: 12pt 'MS Shell Dlg 2'; color: red"
                ...
                self.errorMessage.setText(
                    "You must have a first name, last name, and username"
                )
        else:
            self.errorMessage.setStyleSheet("font: 12pt 'MS Shell Dlg 2'; color: red")
            self.errorMessage.setText(
    ...
    def handleAddPressed(self):
        ...
        if self.type.currentText() and self.pName.text() and self.word.text():
            if not self.ref.exists("prefix", prefix) and not self.ref.exists(
                ...
                self.view.auditor(
                    currentTech, "Edit", self.user.text(), "Settings_Edit_Technician"
                )
            else:
                self.errorMessage.setStyleSheet(
                    "font: 12pt 'MS Shell Dlg 2'; color: red"
...
class RejectionLogForm(QMainWindow):
    def __init__(self, model, view):
        ...
        self.print.clicked.connect(self.handlePrintPressed)
        self.home.clicked.connect(self.handleReturnToMainMenuPressed)
        rejectedCulture = self.model.findRejections(
            "Cultures",
            "[SampleID], [Type], [Clinician], [Rejection Date], [Rejection Reason]",
        )
        rejectedCAT = self.model.findRejections(
            "CATs",
            "[SampleID], [Type], [Clinician], [Rejection Date], [Rejection Reason]",
        )
        rejectedDUWL = self.model.findRejections(
            "Waterlines",
            "[SampleID], [Clinician], [Rejection Date], [Rejection Reason]",
        )
        count = 0
    ...
    def handlePrintPressed(self):
        ...
        for i in range(self.rejLogTable.rowCount()):
            self.printList.append(
                [
                    self.rejLogTable.item(i, 0).text(),
                    self.rejLogTable.item(i, 1).text(),
                    self.rejLogTable.item(i, 2).text(),
                    self.rejLogTable.item(i, 3).text(),
                    self.rejLogTable.item(i, 4).text(),
                ]
            )
...
class AddClinician(QMainWindow):
    ...
    def selectedClinician(self):
        if self.clinDrop.currentText() == "":
            ...
            clinician = self.model.findClinicianFull(
                self.view.entries[self.clinDrop.currentText()]["db"]
            )
    ...
    def handleSavePressed(
        self,
    ):  # Incorporate validation to make sure clinician is actually added to DB
        self.timer.timeout.connect(self.timerEvent)
        self.timer.start(5000)
        title, first, last = "", "", ""
        if (
            self.fName.text()
            and self.lName.text()
            and self.address1.text()
            and self.city.text()
            and self.state.currentText()
            and self.zip.text()
        ):
            ...
            if self.clinDrop.currentText() == "":  # and self.sample is None:
                ...
                self.errorMessage.setText(
                    "New clinician added: " + title + " " + first + " " + last
                )
                self.view.auditor(
                    currentTech, "Create", title + "" + first + " " + last, "Clinician"
                )
            else:
                ...
                self.errorMessage.setText(
                    "Updated Existing Clinician: " + title + " " + first + " " + last
                )
                self.view.auditor(
                    currentTech, "Update", title + "" + first + " " + last, "Clinician"
                )
        else:
            self.errorMessage.setStyleSheet("font: 12pt 'MS Shell Dlg 2'; color: red")
            self.errorMessage.setText("* Denotes Required Fields")
        ...
        if self.clinDrop.currentText():
            if (
                ...
                if self.sample is None:
                    ...
                    self.view.auditor(
                        currentTech, "Create", self.saID.text(), "DUWL_Order"
                    )
                else:
...
class DUWLReceiveForm(QMainWindow):
    ...
    def handlePrintPressed(self):
        template = (
            str(Path().resolve()) + r"\templates\pending_duwl_cultures_template.docx"
        )
        dst = self.view.tempify(template)
        document = MailMerge(template)
...
class CultureResultForm(QMainWindow):
    def __init__(self, model, view):
        ...
        try:
            with open("local.json", "r+") as JSON:
                ...
                self.options = (
                    ["NA"]
                    + list(self.growthList)
                    + list(self.blacList)
                    + list(self.susceptibilityList)
                )
                self.optionIndexes = {
                    "NA": 0,
                    "NI": 1,
    ...
    def threader(self, arg):
        if arg == 0:
            ...
            ui = self.handlePreliminaryPressed
        else:
            ui = self.handlePerioPressed
        self.thread = QThread()
        if self.handleSavePressed():
        ...
        if kind:
            if row < len(self.aerobicTable):
                if column < len(self.aerobicTable[row]):
                    self.aerobicTable[row][column] = (
                        self.aeTWid.cellWidget(row, column).currentText()
                        if self.aeTWid.cellWidget(row, column)
                        else self.aerobicTable[row][column]
                    )
        else:
            if row < len(self.anaerobicTable):
                if column < len(self.anaerobicTable[row]):
                    self.anaerobicTable[row][column] = (
                        self.anTWid.cellWidget(row, column).currentText()
                        if self.anTWid.cellWidget(row, column)
        ...
        if result is not None:
            ...
            for i in range(0, len(result)):
                ...
                if bacteria[0].isnumeric() and int(bacteria[0]) in entryList:
                    table.append(
                        [self.swap.translate(type, int(bacteria[0]), "entry", "word")]
                    )
                else:
                ...
                for j in range(0, len(antibiotics)):
                    ...
                    if i < 1:
                        if (
                            measures[0] != "Growth"
                            and measures[0] != "B-lac"
                            and measures[0].isnumeric()
                        ):
                            abPrefix = self.swap.translate(
                                "Antibiotics", int(measures[0]), "entry", "prefix"
                            )
                            measures[0] = abPrefix
            ...
            for i in range(1, len(table)):
                ...
                for j in range(1, len(table[i])):
                    ...
                    if table[0][j] in self.swap.get("Antibiotics", "prefix"):
                        tmp = self.swap.translate(
                            "Antibiotics", table[0][j], "prefix", "entry"
                        )
                        table[0][j] = str(tmp)
    ...
    def handleSearchPressed(self, data):
        ...
        if data == False:
            if not self.saID.text().isdigit():
                ...
                self.errorMessage.setText("Sample ID may only contain numbers")
                self.errorMessage2.setStyleSheet(
                    "font: 12pt 'MS Shell Dlg 2'; color: red"
                )
            ...
            self.sample = self.model.findSample(
                "Cultures",
                int(self.saID.text()),
                "[ChartID], [Clinician], [First], [Last], [Tech], [Collected], [Received], [Reported], [Type], [Direct Smear], [Aerobic Results], [Anaerobic Results], [Comments], [Notes], [Rejection Date], [Rejection Reason]",
            )
            if self.sample is None or len(self.saID.text()) != 6:
                self.handleClearPressed()
                self.saID.setText("xxxxxx")
        ...
        if (
            ...
            if (
                ...
                if self.model.addCultureResult(
                    ...
                    self.errorMessage.setText(
                        "Saved Culture Result Form: " + self.saID.text()
                    )
                    self.errorMessage2.setStyleSheet(
                        "font: 12pt 'MS Shell Dlg 2'; color: green"
                    )
                    ...
                    self.view.auditor(
                        currentTech, "Update", self.saID.text(), "Culture_Result"
                    )
                    return True
            else:
                self.errorMessage.setStyleSheet(
            ...
            if float(self.collectionTime.text()) != 0:
                if (
                    ...
                    if self.model.addCATResult(
                        ...
                        self.view.auditor(
                            currentTech, "Update", self.saID.text(), "CAT_Result"
                        )
                        return True
                else:
                    self.errorMessage.setStyleSheet(
